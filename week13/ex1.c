...
int main(int argc, char *argv[])
{
	...
	int processes[MAX_PROCESSES], existing_resources[MAX_RESOURCES], available_resources[MAX_RESOURCES];
	int current_allocation[MAX_PROCESSES][MAX_RESOURCES], request[MAX_PROCESSES][MAX_RESOURCES];
	
	// ------- START INPUT --------------
	
	// line 1 - existing resources (E array)
	...
	// --------- START ALGORITHM -------------
	int process_ok[MAX_PROCESSES] = {};		// bool,
	...
	while(1)
	{
		...
		for(int i=0; i<process_count; i++)
		{
			...
			if(j != available_count)	// cannot give resources to this process
				continue;				// check next

			// process ran & terminated. now give back its resources
			for(j=0; j<available_count; j++)
			{
				available_resources[j] += request[i][j] + current_allocation[i][j];
			}
			satisfied = 1;	// at least one process got what it wanted
			process_ok[i] = 1;
		}
		if(satisfied == 0)
			break;
